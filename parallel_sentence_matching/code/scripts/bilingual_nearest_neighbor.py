...
def get_embeddings_as_array(w2v: KeyedVectors):
    # w2v.vectors: (n, dim) in derselben Reihenfolge wie w2v.index_to_key
    vecs = w2v.vectors.astype("float32")
...
def main(source_embeddings, binary, output, target_embeddings=None, knn=10,
    ...
    with open(output, 'w', encoding='utf-8') as fout:
        for i, idxs in enumerate(indices):
            # source word/id
            sw = se.index_to_key[i]
            # take only best target
            tgt_idx = idxs[0]
            tgt_word = te.index_to_key[tgt_idx]
            score = format(distances[i][0], f'.{float_precision}f')
            # ✅ source first, target second
            fout.write(f"{sw}\t{tgt_word}\t{score}\n")
